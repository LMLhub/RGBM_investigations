...
\begin{document}
\begin{titlepage}
\title{Mixing Time as a Measure for Economic Mobility: The Case of Reallocating Geometric Brownian Motion}
...
%\ec
\begin{abstract}
\noindent We introduce mixing time as a measure of economic mobility. Mixing times quantify the characteristic time scale over which individual incomes or wealths mix into their distribution in a population. We study the properties of mixing time by studying mobility in Reallocating Geometric Brownian Motion (RGBM), an established model of wealth in a growing economy. We describe the relationship between finite mixing times and standard measures of economic mobility. Measuring mobility using the standard measures in a non-ergodic system, in which the mixing time is infinite, may lead to misleading conclusions about the extent of mobility in an economy. We also find that RGBM recovers several stylized facts, such as an inverse association between mobility and inequality and the typical shape of intertemproal copulas.
\\
...
Mobility measures are typically derived from the joint distribution of wealth at two points in time. On this basis, \citet{Shorrocks1978} described several required properties for the statistical measures of mobility and set the standard for such measures. He suggested that mobility measures should be normalized, monotonic and independent of the observation period\YB{the last two criteria are not clear. perhaps better not to mention them at all if this demands expanding too much}. Any measure defined in a such manner will, however, represent an aggregate value for the extent of mobility in the economy, and it is possible that it may fail to describe the mobility of the typical individual.
...
%\textcolor{red}{The standard statistical measures of mobility have several limitations. First, they are generally incomparable. For example, a rank correlation of 0.3 would have a different meaning if it corresponds to a period of one year or ten years. Second, for a given time period, it is not possible to tell whether some value of rank correlation is high or low, as this is a dimensionless quantity. Thus, the rank correlation over some time period can only be high or low in comparison to other economies over a similar time period, or to other time periods of the same length.}

%\textcolor{red}{In addition, the interpretation of the rank correlation depends on the underlying wealth distribution and its dynamics. The same rank correlation cannot be interpreted in the same manner when there is no change in the underlying wealth distribution, and when the distribution of wealth becomes more and more unequal. The IGE has similar limitations, but also others. Most notably, the IGE is sensitive by design to the level of inequality and not only to the transition matrix, as discussed in detail in the mobility literature (\eg \citet{chettyETAL2014}).}

% This paper
This paper defines the property of \textit{ergodicity} for statistical mobility measures. Any ergodic mobility measure quantifies the feasibility of the individuals to move across the entire steady-state wealth distribution. It thus captures the mobility of the typical individual in the economy.\YB{This is not discussed later on in the same way, and we don't formally define what is an ergodic mobility measure, so I'm not sure it is necessary to point that out}
We thereby introduce mixing time, a feature of stochastic processes, as a measure of mobility that satisfies this property. When rescaled wealth (individual wealth divided by the sample mean wealth) is an ergodic observable \citep{PetersAdamou2018c}, and assuming the rescaled wealth distribution approaches a steady state, the distribution of wealth of every arbitrary subgroup of individuals this group will gradually become similar to the steady-state distribution. The mixing time is the characteristic time of this convergence. Put simply, it is the time scale over which individuals mix into the wealth distribution. When mixing is rapid, \ie the mixing time is short relative to the window of observation, we could interpret that as high wealth mobility. Slow mixing is interpreted as low mobility.
...
\textbf{Measure }  & \multicolumn{5}{c}{\textbf{Property}} \\\hline
           & Normalization & Monotonicity & P. dependence & D. dependence & Ergodicity \\\hline
Mixing time          & $\times$             & $\times$            & $\times$                 & \checkmark                       & \checkmark          \\
Spearman Correlation & \checkmark             & \checkmark            & \checkmark                 & $\times$                      & $\times$          \\
IGE                  & \checkmark             & \checkmark            & \checkmark                 & \checkmark                       & $\times$          \\
Gumbel parameter     & \checkmark             & \checkmark            & \checkmark                 & $\times$                       & $\times$         
\end{tabular}}
\end{table}

Mixing time is also the only measure that does not satisfy the monotonicity and the period dependence properties. Monotonicity implies that if there is mobility between certain quantiles of the wealth distribution, it will be translated with existence of mobility in the standard measures. All other measures represent aggregated values of the changes in the wealth rankings of the individuals which constitute the population between two time periods. Thus, if the transition matrix is slightly perturbed between the two periods, it will result in change in the magnitude of mobility predicted by the measure. Mixing time, on the other hand, is not monotonic unless there is already mobility between every quantile. In every other case it does not exist.

Next, we study the distribution dependence property of the measures. This property is captured by mixing time. Also, the IGE is dependent on the shape of the wealth distributions that are used for its estimation, whereas the other two measures are not. This dependence indicates that mixing time will not be interpreted similarly when the underlying wealth distribution remains unchanged, and when it becomes more and more unequal.
...
\paragraph{Intragenerational elasticity:} Similarly to the properties of the rank correlation and as evidenced in~\fref{rgbm-standard-measures}B, the IGE depends on both the noise amplitude and the magnitude of reallocation.
...
\textcolor{red}{\paragraph{Transition matrices:}} As evidenced in~\fref{rgbm-wealth-matrices}A, the transition matrices in RGBM reproduce the asymmetric property of the real world transition matrices and are well-approximated by the Gumbel copula (\fref{rgbm-wealth-matrices}B) \citep{JanttiJenkins2015}.
%The major advantage of modeling through the Gumbel copula is that this copula is uniquely identified by one dependence parameter $\theta$: a larger dependence implies less mobility.

In \fref{rgbm-wealth-matrices}C we visualize the relationship between Gumbel parameter $\theta$ and the reallocation rate $\tau$ for various noise amplitudes. We find that there is an inverse relationship between $\theta$ and $\tau$, whose slope is determined by the magnitude of $\sigma$. As $\tau$ increases, the value of the $\theta$ decreases, though disproportionately. A similar picture arises when we consider the rank correlation over $\delta = 20$ as a function of $\tau$, depicted in \fref{rgbm-wealth-matrices}D.\footnote{We note that there is a direct mapping between $\theta$, the parameter of the Gumbel copula, and the rank correlation represented by the same copula. Yet, this mapping cannot be analytically represented.}

\begin{figure}[!htb]
\centering
\includegraphics[width=1.0\textwidth]{figs/fig_rgbm_gumbel_v2.eps}
\caption{\textbf{Mixing time and wealth transition matrices.} \textbf{A)} Transition matrix for the stationary regime of RGBM estimated with $\tau = 0.02$ per year, $\sigma^2 = 0.01$ per year and $N = 10^4$ people. \textbf{B)} An example for a transition matrix from data simulated from a Gumbel copula whose parameter $\theta$ is chosen to be in accordance with the RGBM parameters used in \textbf{A)}. \textbf{C)} The relationship between the Gumbel copula parameter $\theta$ and the reallocation rate $\tau$ in the stable state of RGBM. \textbf{D)} The relationship between the Gumbel copula parameter $\theta$ and the rank correlation $r$ in the stable state of RGBM. The parameters were estimated from a transition matrix in which $\delta = 20$ years and $N = 10^4$ people.
...
\ee
%
\fref{rgbm-great-gatsby} displays this relationship, demonstrating that RGBM reproduces the qualitative empirical observation that inequality and immobility are positively related.\footnote{We note that in the context of this paper this relationship is intragenerational and not intergenerational.}
...
\paragraph{Intragenerational elasticity:} The intragenerational elasticity is defined as the slope $b_{t_m,t_n}$ of the regression
%
\be
...
\section{Derivation of IGE in RGBM}

As a measure, the intragenerational elasticity is determined by the dynamics of the log of (rescaled) wealth $z = \log(y)$. Using Ito calculus, we can recover the following equation for the dynamics of $z$,
\begin{align*}
