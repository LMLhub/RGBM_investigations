...
We use RGBM to numerically present the procedure described in~\Sref{mixing-time}. In the concrete example, we focus on the role of the subsample size, $\tau$ and $\sigma$ in the duration of the mixing time period and the estimation of the mixing time measure. For this purpose, in~\Fref{rgbm-mixing-time} we plot the log of Kolmogorov-Smirnov (KS) statistic as a function time and vary the subsample size, reallocation rate and the noise amplitude. Intuitively, the reallocation rate has the major role in determining the mixing time, whereas the noise amplitude has no effect, as argued in~\Sref{rgbm}. However, it appears that the subsample size critically determines the behavior of the transient regime in the system. While this initial finding is interesting, we believe that its exact properties are out of the scope of the paper.


\begin{figure}[!htb]
\centering
\includegraphics[width=1.0\textwidth]{figs/fig_mixing_time_rgbm.eps}
\caption{\textbf{Mixing time in RGBM.} The figure shows the log of KS statistic as a function of time for a realization of an RGBM process with a population size of $10^6$. The two panels correspond to different subsample sizes used for the estimation of the KS statistic ($10^4$ and $10^5$). The black lines are estimates with $\tau = 0.01$ and the black lines are with $\tau = 0.02$. Similarly, the dashed lines are estimated with $\sigma^2 = 0.01$ and the dotted lines with $\sigma^2  =0.015$. \label{fig:rgbm-mixing-time}}
\end{figure}
