...
We consider five properties that a mobility measure may have. First, following \citet{Shorrocks1978} we identify the properties of 1) \textit{normalization} - the values that the mobility measure may take are bounded in a closed interval; 2) \textit{monotonicity} - if a new structure is imposed in the wealth dynamics of some individuals then this is reflected in the value of the measure, and 3) \textit{period dependence} - the mobility predicted by the measure is dependent on the temporal difference $\delta$ between the periods that are used for its estimation. In addition, we follow \citet{cowell2018measuring}, and add to our analysis the property of 4) \textit{distribution dependence} which evaluates whether the mobility predicted by the measure is dependent on the shape of the empirical wealth distributions that are used for its estimation. %Lastly, we introduce the property of 5) \textit{mixing} which, as argued above, quantifies whether the mobility measure accounts for the feasibility of moving across the whole wealth distribution, \ie if all individuals are able to change their wealth status.

Table~\ref{tab:mobility-properties} reports the properties that are captured by the three standard mobility measures that we study. For comparison, the table also gives the properties that are captured by mixing mobility measures and, for illustrative purposes, adds a column for the mixing property. We see that mixing measures may not satisfy the normalization property. Also, the IGE does not satisfy this property. However, we state that normalization is just a standard procedure that can be implemented to any measure, and measures that account for mixing can be easily adjusted to satisfy this property.
...
\textbf{Measure }  & \multicolumn{5}{c}{\textbf{Property}} \\\hline
           & Normalization & Monotonicity & P. dependence & D. dependence & Mixing \\\hline
Mixing measures         & ?             & $\times$            & $\times$                 & \checkmark                       & \checkmark          \\
Spearman Correlation & \checkmark             & \checkmark            & \checkmark                 &  $\times$                      & $\times$          \\
IGE                  & $\times$             & \checkmark            & \checkmark                 & \checkmark                       & $\times$          \\
