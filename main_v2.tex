...
Table~\ref{tab:mobility-properties} details the properties above for the three standard mobility measures we study. For comparison, the table also gives the properties that are captured by mixing mobility measures (\YB{again, unclear what this means}) and, for illustrative purposes, adds a column for the mixing property. We see that mixing measures may not satisfy the normalization property. Also, the IGE does not satisfy this property. However, we state that normalization is just a standard procedure that can be implemented to any measure, and measures that account for mixing can be easily adjusted to satisfy this property (\YB{How?})
